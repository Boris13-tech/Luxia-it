...
- All three contact flows reach review and produce a correctly encoded mailto draft to Contact@legrand-tech.com. English and Chinese bodies are localized. No email or WhatsApp message was sent.
...
## Expertise interactive experience — 9 September 2026

Scope: `/fr/expertise`, `/en/expertise`, `/zh/expertise`. Existing service detail pages and home strategy are preserved. The shared renderer accepts opt-in experience parameters; other uses keep their existing behavior.

A single mounted canvas responds to four capability selectors. Intelligence changes connection paths; Trust closes gates and isolates refused packets, supports a shorter privileged access window, and returns telemetry; Scale increases distribution; Automation gives selected exceptions an alternate path. The five existing Framework stages now also drive fragmentation, organization, assembly, trust and expansion. The technical reveal explains separate identity, optional PIM, workload authorization, managed identity/Key Vault and Sentinel telemetry flows, with Microsoft Learn references. It is explicitly a concept simulation.

Desktop (1440×900) and mobile (390×844) reviewed. Fixed a transient whole-scene darkening when switching modes, undersized mobile geometry and obscured mobile controls. The mobile scene stays above controls; horizontal overflow was absent and one canvas remained mounted. Refusal and temporary privilege controls and Discover/Evolve panels were exercised. Animation pauses after 45 seconds without interaction and resumes on pointer, keyboard or scroll activity, in addition to visibility/reduced-motion handling. An old development browser tab was found crashed after an unattended interval; cause was not established. Fresh-tab checks completed; no claim of measured field performance is made.

Technical references verified: Microsoft Learn Conditional Access overview; PIM overview; Key Vault authentication; Microsoft Sentinel data connectors. Conditional Access is not described as a network proxy, PIM is optional and time-bound, and managed identity authentication is separate from the user session.
